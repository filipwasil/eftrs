...
trait WeaponCaps
{
    fn get_ammo() -> std::boxed::Box<dyn Ammo::Ammunition>;
    // fn get_silencer();
    // fn get_foregrip();
    // ...
}
...
impl WeaponCaps for VeprAKMVPO209
{
    fn get_ammo() -> std::boxed::Box<dyn Ammo::Ammunition>
    {
...
struct LoneStarTX15 {}
impl WeaponCaps for LoneStarTX15
{
    fn get_ammo() -> std::boxed::Box<dyn Ammo::Ammunition>
    {
        std::boxed::Box::new(Ammo::AK_5_56x45)
...
// }
    ...
    // pub fn get_weapon(&self, weapon_name: &str) -> std::result::Result<&std::boxed::Box<dyn Ammo::Ammunition>, &str> {
...
impl<'a> Weapon<'a>
{
    ...
    pub fn get_weapon_impl(&self, weapon_name: &str) -> std::result::Result<&std::boxed::Box<dyn WeaponCaps>, &str> {
        match weapon_name
        {
            "ADAR215" => Ok(std::boxed::Box::new(Ammo::Pistol_9x19)),
            _ => Err("Not found")
        }
    }

    pub fn get_weapon(&self, weapon_name: &str) -> std::result::Result<&std::boxed::Box<dyn WeaponCaps>, &str> {
        let result = self.get_weapon_impl(weapon_name);
        if let Some(ammunition) = self.get_weapon_impl(weapon_name) {
            return Ok(ammunition);
        }

        println!("\nWeapon not found. Trying to find one that may fit...\n");
        if let Ok(weapon) = self.suggest_closest_match(String::from(weapon_name)) {
            if let Some(weapon) = self.get_weapon_impl(weapon.as_str()) {
                return Ok(weapon);
            }
        }
